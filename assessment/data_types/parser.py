	...
	for line in file:
		...
		match section:
			...
			case "projects_summary":
				...
				splits = line.split();
				if splits:
					data.projects_summary.append(parse_types.ProjectSummary(
			...
			case "precedence_relations":
				...
				splits = line.split()
				if(splits):
					data.precedence_relations.append(parse_types.Job(
			...
			case "durations_resources":
				...
				if(splits):
					...
					data.durations_resources.append(parse_types.DurationResource(
						job_number = int(splits[0]),
						mode = int(splits[1]),
